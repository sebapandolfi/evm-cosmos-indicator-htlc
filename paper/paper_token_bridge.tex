...
\begin{enumerate}
    \item We design and implement an \textbf{atomic cross-chain bridge for environmental indicator tokens} using ERC-1155/CW1155 multi-token standards with deterministic indicator identity, cryptographic metadata commitment, and governance-controlled issuance to preserve semantic integrity.
...
\textbf{Gravity Bridge}~\cite{gravity2022} connects Ethereum to Cosmos using a dedicated validator set. Unlike our approach, it requires maintaining separate validators and lacks explicit atomicity guarantees. It provides no mechanism for semantic binding of heterogeneous token types.
...
\textbf{Wormhole}~\cite{wormhole2024} implements a guardian network for cross-chain attestation. Our approach adds an explicit HTLC layer with automatic callback for atomicity and semantic binding for indicator integrity.
...
This section presents the architectural design of our system, including the semantic binding mechanism for environmental indicators, the HTLC protocol with automatic GMP callback, and the cross-chain consistency guarantees.

\subsection{Semantic Binding and Governance Controls}
\label{sec:semantic-binding}

To solve the semantic mixing problem described in Section~\ref{sec:indicators}, our design binds each token class to a unique environmental indicator identity using cryptographic commitments and immutable on-chain registries.

\textbf{Cryptographic metadata commitment.} Each environmental indicator is described by an off-chain profile containing the indicator type, canonical unit, methodology identifier, and evidence data. Two cryptographic hashes anchor this profile on-chain: \texttt{profileHash} (hash of the full indicator profile) and \texttt{dataHash} (hash of the evidence data bundle). A deterministic \texttt{indicatorId} is derived as:
\begin{equation}
...
Our system connects EVM and Cosmos ecosystems through Axelar's GMP protocol:
...
The semantic binding design (Section~\ref{sec:semantic-binding}) provides the following guarantees for environmental indicator integrity:
...
\begin{enumerate}
    ...
    \item \textbf{Off-chain profile storage}: The full indicator profile and evidence data are stored off-chain; only their cryptographic hashes are on-chain.
...
\begin{enumerate}
    ...
    \item \textbf{Decentralized governance}: Replace owner-based token class creation with DAO-based voting.
    \item \textbf{On-chain profile registry}: Store indicator profiles on decentralized storage (e.g., IPFS with on-chain CIDs).
...
This paper presented the design, implementation, and production deployment of an atomic cross-chain bridge for environmental indicator tokens connecting EVM and Cosmos ecosystems. Our approach combines ERC-1155/CW1155 semantic binding with deterministic indicator identity and governance-controlled issuance to preserve the environmental meaning of tokens across heterogeneous chains, solving the semantic mixing problem inherent in naive ERC-20 bridging.

We introduced an HTLC protocol with automatic callback that provides strong atomicity guarantees: minting on Cosmos automatically triggers burning on EVM, eliminating the timeout race condition of standard HTLC designs. A comprehensive security audit mitigated 10 vulnerabilities across both contract layers.
